<form action="{{ route('customer.order.shipment.finalize', ['order' => $order]) }}" method="post">
    ...
    <div class="mt-5">
        ...
        <div class="row my-3">
            <select class="select-description" name="shipment" id="shipmentOption" required>
                @foreach ($shipments as $key => $shipment)
                    <option value="" disabled selected>Select Shipment</option>
                    <optgroup label="{{ $shipment['name'] }}">
                        @foreach ($shipment['costs'] as $key => $service)
                            <option data-price="{{ $service['cost'][0]['value'] }}"
                                data-description="{{ config('payment.currency_symbol') . $service['cost'][0]['value'] . '(' . $service['cost'][0]['etd'] . ' days)' }}"
                                value="{{ implode(':', [$shipment['code'], $service['service']]) }}">
